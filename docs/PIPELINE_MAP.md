...
첫 시험의 "검사 완료 후 전략 실행" 절차에는 대용량 검사 전용 계획이 별도로 필요하다.
현재 직접 CLI를 검사 전용 도구로 이름만 바꾸거나 작은 화면/압축 시제품의 상한을 풀어 대체하지 않는다.
담당 체크 항목은 [BACKTEST_TODO §3](../BACKTEST_TODO.md#3-장외-실행-전-준비)에 있다.
...
**첫 연구 전:** 기존 raw reader와 품질 정책을 공유하는 검사 전용 절차를 확정한다.
입력 identity·전체 소진 여부·첫 품질 오류·코드 해시·검증 범위를 남기고, 스캔 중단을 합격으로 바꾸지 않아야 한다.
이미 확인한 종료/표본 근거를 다시 스캔해 대체하지 않는다. 신규 수집 시작 자체를 이 개발의 완료에 묶지 않는다.
