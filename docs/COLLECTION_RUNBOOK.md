...
<a id="collection-silence-profile"></a>
명시적 프로필 감시에서는 **경고·회복·종료 보고가 같은 구간/임계/종류별 시계**를 쓴다.
체결 침묵은 체결 수신으로만, 호가 침묵은 호가 수신으로만 회복된다. 호가만 재개돼도 체결 침묵은 열린 채로 남는다.
침묵을 판정한 구간이 닫히면 열린 침묵은 그 경계에서 미회복으로 닫히고, 다음 판정 구간은 개장 시각부터 새로 잰다.
종료 보고는 마지막 같은 종류 수신(없으면 감시 시작)~종료 중 판정 구간과 겹친 길이만 각 구간 임계와 비교한다.
`NOT_EXPECTED`/`UNJUDGED` 구간, 구간 밖 시간, 감시 시작 전 시간은 어느 단계에서도 결손으로 세지 않는다.
종료 권고(`silence_stop_reason`)는 프로필 경로에서 체결 기준이며 체결 수신으로만 풀린다.
legacy `sessions=None` 경로의 any-event 의미는 바꾸지 않았다. 둘 다 저장 완료·coverage 인증이 아니다.

<a id="collection-queue-backlog"></a>
**대기큐 적체 경보**는 넘침 fail-closed 보호와 별개인 경고다. raw-v2 큐는
[`live_capture.py`](../collector/kiwoom/live_capture.py)의 `QUEUE_CAPACITY`(8,192)·`QUEUE_BATCH_SIZE`(512)를 쓰며,
기본 바닥은 `queue_backlog_floor_for_capacity`로 capacity의 절반(4,096)이다. 구조적 임시 정책값이고 실측 조정값이 아니다.
capacity보다 큰 바닥은 도달할 수 없으므로 설정 오류로 거부한다. capacity가 없는 raw-v1 무한 큐만 기존 20,000을 쓴다.
판정은 바닥 이상이 처음 관측된 시각부터 `stuck_window` 이상 이어지고, 창 경계 이하 표본 중 가장 최근 기준점보다
줄지 않았을 때만 경보한다. 표본 간격이 불규칙하거나 창보다 길어도 기준점을 버리지 않는다. 바닥 아래 표본이나
시계 역행은 지속 시간을 새로 잰다. 큐가 실제로 줄고 있으면 경보하지 않는다.
