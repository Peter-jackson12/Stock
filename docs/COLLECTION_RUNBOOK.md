...
아래는 `5b5156f`의 코드 대조에 근거한 운영 계약이다. 현재 날짜의 휴장/특별 개장이나 실제 수신을 인증하지 않는다.
...
접미사 힌트가 있어도 venue=unknown, coverage=unconfirmed를 유지한다. 純수 함수 자체는 로그인/파일 저장을 하지 않는다.
...
- 주식수 배수 `1000`은 사용자 제공 삼성전자 표본과 정합성 검사 통과에 근거한 명시적
...
이는 운영자 진술이며 도구가 근거 내용을 자동 인증하는 기능이 아니다. `closed` 헤더만으로 승인하지 않는다.
...
- `--environment`는 실행 위치의 자기 신고다. 프록시 환경변수 유무는 기록하지만
