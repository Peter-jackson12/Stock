...
if selection.is_multi:
    st.plotly_chart(cumulative_pnl_by_run_chart(daily_performance_by_run(df)), use_container_width=True)
else:
...
show = daily[["date", "trade_count", "avg_pnl", "win_rate", "avg_mae_pct", "total_pnl"]].tail(30).copy()
