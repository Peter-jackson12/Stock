...
운영 진입점의 기본 raw v2 콜백·종료/저장과 상태 관측을 연결했다. 화면 제어·운영 IPC는 후속이다.
...
파일은 생존/데이터 품질 인증이나 명령 응답이 아니다. 화면은 raw DB를 열거나 수집기를 기동하지 않는다.
운영 IPC의 명령 전달·상시 heartbeat 계약과 관리자 기동은 후속이다.
...
4. **3 통과 후:** 화면/관리 프로세스에 수집기 시작/종료 제어와 종료 데이터 검증을 연결한다.
