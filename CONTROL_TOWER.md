...
4. **작업 이력:** 대기/계획 단계만 취소한다. 이미 선점된 작업은 이 버튼으로 종료하지 않는다.
   **진행 중·대기 작업**(planned/queued/running)은 최근 이력 개수와 무관하게 오래된 순으로 최대 100건,
   **최근 종료 이력**은 기존대로 30건만 보인다. 같은 요청이 미완료면 기존 작업 ID를 돌려주며 앞 목록에서 찾는다.
...
**Popen 성공은 launch 성공이 아니다.** 부모는 Popen handle 이 PID 재사용을 막는 동안 자식의 PID·OS 생성 시각·
실행 파일을 `spawns` 표에 고정한다(소유권 근거로는 쓰지 않는다). 자식이 claim 전에 끝나면 요청은 `launching`에
남지 않고 `failed`로 수렴한다: 생성 직후 이미 종료했으면 즉시, 그 뒤면 관리 화면의 5초 갱신이나 이력 대조가
고정 identity 소멸(없는 PID 또는 생성 시각/실행 파일이 다른 PID 재사용)을 확인할 때다. 같은 identity 가
살아 있으면 기다리고, 접근 거부 등은 종료 증거가 아니다. 전환은 `owner IS NULL AND state='launching'` 조건의
단일 갱신이라 capability 도 함께 무효화되고, 늦게 깬 자식은 claim 하지 못한다. 실패 원인에는 launch 로그 끝
최대 768 bytes 를 1,024자 안에서 남긴다. identity 를 고정하지 못한 요청은 추측하지 않고 기존처럼 취소로 푼다.
Popen 예외는 기존대로 `unknown`이다. kill·재시작·재로그인·잠금 삭제는 추가하지 않았다.
