...
- `CaptureHistory(root)`는 `operations_state/capture_control.sqlite3`에 등록된 identity,
  ...
  실제 전송 여부는 이 반환만으로 알 수 없다. 전송 큐나 자동 재시도 기능은 없다.
...
- 복구 시 과거 monotonic 시각과 생존 정보는 폐기한다. 새 연속 revision을 받아야 현재 상태를
  ...
  다시 받아야 하며 최신 snapshot 하나로 건너뛰지 않는다. 그 전송 경로는 아직 없다.
...
핸들 하나의 호출은 순차 실행해야 한다. DB 소유권 세대는 외부 전송을 차단하지 못하므로,
미래 전송 계층도 소유권을 검사해야 한다. 이미 반환된 명령은 회수할 수 없다.
...
UI·프로세스·IPC 연결은 별도 단계다. 최신 실행 수치는 HANDOFF에 둔다.
...
2. **구현/합성 검증:** 관리 대상 식별/명령/응답 계약, 가짜 피어의 종료·이전 세션 메시지 차단.
   명령 이력 영속화·관리자 재시작 대조도 합성 검증했다. 다음은 전송 소유권·누락 보고 복구 계약이며,
   그 뒤 IPC/실제 프로세스 어댑터를 연결한다.
