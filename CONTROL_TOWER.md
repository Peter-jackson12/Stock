...
실제 수집기 제어/IPC, 관리 프로세스의 재시작 복구, 운영 raw v2 연결은 후속이다.
...
이 단계는 **단일 소유자가 순차 호출하는 메모리 상태 머신과 메시지 계약**이다.
OS PID 확인, 프로세스 기동/종료, 소켓/파일 IPC, 디스크 여유 관측, 명령 이력 영속화,
manager 재시작 후 대조 복구는 구현하지 않았다. PID/로그로 상태를 복원하거나
...
2. **구현/합성 검증:** 관리 대상 식별/명령/응답 계약, 가짜 피어의 종료·이전 세션 메시지 차단.
   독립 후속은 명령 이력 영속화·관리자 재시작 대조 계약이다. 그 뒤 IPC/실제 프로세스 어댑터를 연결한다.
