...
| # | 내용 | 처리 |
| :---: | :--- | :--- |
| 5 | **유니버스 정의 3중 불일치** (LOB 1,300~1,500 / fs_v1 200 / 엔진 3). 일봉 매트릭스가 유니버스를 암묵적으로 결정 | §3.6.1 추가 발견 참조 — **최우선** |
| 6 | **`upper`/`tick_rate` 매핑표 오류.** 이름은 비슷하나 계산 기준이 다름 | §3.7.1 정정 완료 |
| 7 | **20260908 LOB DB의 `vol` 등이 BLOB** — 피처 빌드 불가, 양쪽 경로 모두 실패. L1(`build_lob_db.py`) 데이터 품질 문제 | 미해결 — **전 종목 수집 전 필수** |
| 8 | `engine.py:421` 예외 핸들러가 `stock` 할당 전 참조 → `UnboundLocalError` (B-1 이전부터 존재) | 미해결 (소) |
...
| **B-2** | **fs_v2 저장 포맷 개선** — float32 / `time` int32 / 거시 피처 일별 분리 (§3.5.2) | `features/store.py`, `fs_v2/` | 中 | B-1 | ⏸️ **보류** — 전 종목 수집으로 용량이 실제 제약이 될 때 재개 (사유: 진행 로그 3차) |
| **B-3** | **거시 null 정책 + 커버리지 게이트, 해상도 표기 정합** (§3.6.1, §3.6.2) | `features/builders/macro.py`, `scripts/build_features.py` | 小 | B | ✅ 완료 (2026-09-14) |
| **B-4** | **유니버스 단일 소스화** — 일봉 매트릭스가 암묵적으로 정하던 유니버스를 명시 선언으로. 선언 vs 실처리 대조 게이트 (§3.6.1 추가 발견) | `engine/engine.py`, `universe.yaml` | 中 | B-1 | ⬜ 미착수 · **최우선** |
| **B-5** | **L1 데이터 품질 — 20260908 BLOB 문제** 및 `engine.py:421` 핸들러 버그 | `collector/build_lob_db.py`, `engine/engine.py` | 小~中 | — | ⬜ 미착수 · 전 종목 수집 전 필수 |
...
**권장 착수 순서** (2026-09-14 3차 재조정): `A ✅ → B ✅ → F 임시 ✅ → B-1 ✅ → B-3 ✅ → **B-4 → B-5** → C → D → E` · *(B-2는 보류 — 전 종목 수집 이후)*
