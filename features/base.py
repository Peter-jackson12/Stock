...
class MacroFeature(Feature):
    ...
    하루 중 값이 변하지 않으므로 stream() 은 상수를 반환하는 상태를 돌려준다.
    (기본 구현 제공 — 하위 클래스는 batch 만 구현하면 된다)
    ...
    def _daily_value(self) -> float:
        """
        TODO(Phase B): 전일 확정 일봉 매트릭스에서 당일용 값 1개를 뽑는다.
        하위 클래스가 오버라이드.
        """
...
class FeatureSet:
    ...
    def resolve_order(self) -> list[Feature]:
        """
        TODO(Phase B): deps 를 위상 정렬해 계산 순서를 정한다.
        (피처가 다른 피처에 의존할 수 있으므로)
        순환 의존은 즉시 에러로 보고할 것.
        """
