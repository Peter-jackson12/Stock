...
작성: 2026-09-16 · 역할: 데이터/재생 설계 기준. 최신 구현/검증 상태는 HANDOFF의 현재 인계를 따른다.

운영 관리의 설계와 현재 구현은 [CONTROL_TOWER.md](CONTROL_TOWER.md)를 함께 본다.
문서 길잡이는 [README](README.md), 최신 진행 상태는 [HANDOFF](HANDOFF.md)의 현재 인계다.
기존 Streamlit에 로그 관측·독립 워커의 결과 조회·장외 재생 계획 저장·작업 이력을 추가했다.
수집기 제어와 무거운 재생 실행은 아직 연결하지 않았다. 아래 목표 구조 전체의 완료를 뜻하지 않는다.
