...
정상 사용에서 기억할 기본 진입점은 **`.\stock.cmd ui` 하나**로 모은다. 프로젝트가 운영 단계에 도달하면
같은 안전한 진입점을 호출하는 Windows 바로가기/아이콘을 제공해 더블클릭으로 열 수 있다. 이번 단계는
Run Plan 검토/저장까지만 구현하며 exe 패키징이나 새 자동 실행 권한은 추가하지 않는다.
