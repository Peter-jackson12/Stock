...
- 수집 제어 계약: `control_tower/lifecycle.py`, `tests/test_capture_lifecycle.py`.
  ...
  누락 보고·PID 재사용·저장 실패·이전 관리자 쓰기를 거부하는가? 운영 IPC는 미연결이며 합성 자식으로 검증한다.
...
- 운영 OCX 콜백에서 단조/UTC 시각 부여·원문 추출·큐 연결을 기본 경로에 반영했다.
  `tests/test_live_collector.py`는 가짜 OCX와 실제 SQLite로 실패/종료를 검증한다.
  실제 콜백을 받은 검증이나 전체 유니버스 부하 검증은 아직 수행하지 않았다.
- 접속/파싱 오류 등 제어 이벤트는 prototype_2의 공통 seq 기록과 연구 실패 처리로 추가했다.
  동기식 CaptureSession은 callback_error도 보존한다. 운영 콜백은 큐만 호출하며 이 클래스를 직접 호출하지 않는다.
  오늘 장중 데이터는 이전 raw v1이므로 새 제어 이벤트가 이미 저장됐다고 가정하지 않는다.
