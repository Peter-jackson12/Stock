"""Run NXT smoke replay from an eligible selected-prefix v2 overlay.

This command is for pipeline validation only. It does not certify strategy
performance, the whole raw session, NXT venue identity, or live readiness.
"""
...
def parser():
    ...
    p.add_argument("--selected-prefix-report", type=Path, required=True)
    ...
    p.add_argument(
        ...
        type=instrument,
        required=True,
        help="currently exactly one CODE=VENUE matching the selected overlay",
    )
...
def main(argv=None):
    ...
    try:
        ...
        print(f"NXT selected-prefix smoke failed: {exc}", file=sys.stderr)
        return 2
    print(result)
    return 0
